...
CompassDirection -> s_obj "south"
                    with door_dir s_to, name 's//' 'south',
      				description [;
      					if (player in s_tunnel) <<Examine runes>>;
      					if (player in w_tunnel) "It's too dark to see into the room.";
						if (player in secret_tunnel2) {
...
CompassDirection -> se_obj "southeast"
                    with door_dir se_to, name 'se' 'southeast',
      				description [;
						if (player in secret_tunnel1) {
							if (orb in secret_tunnel2 or secret_chamber or sa_bobelisk) "The alcove towards the southeast is emanating a soft red glow.";
						}
      					"You see nothing unexpected in that direction.";
      				];
...
CompassDirection -> u_obj "up above"
                    with door_dir u_to, name 'u//' 'up' 'ceiling' 'above' '7840placeholder',
                    description [;
						if (player in unholy_shrine) {
							if (huge_boulder.boulderfalling) "Dust and dirt hits your face as a thundering shadow comes into view...";
						}
						if (player in desert or dunes or chasm) <<Examine twin_moons>>;
						if (player in m_base or desert_crash or view_plat or vplat_seats) "The blazing hot sun is blinding to look at, as it beats down upon you burning your flesh.";
						if (player in tranquility_chamber) {
							if (orb in tc_shrine) "The cobalt-blue force field blocks your view.";
							"The thin white haze of the sky doesn't burn in this room, as the sun almost seems diffused as it enters.";
						}
						if (player in north_cell or south_cell or east_cell or west_cell) {
							if (orb in tc_shrine) "The cobalt-blue force field blocks your view.";
							"The sun shines down on top of the shrine, but misses you within this chamber.";
						}
						"You see nothing unexpected in that direction.";
                    ];
CompassDirection -> d_obj "ground"
                    with door_dir d_to, name 'd//' 'down' 'floor' 'below' 'ground' 'deck',
                    description [;
                    	if (player in view_plat) "Stone carved seats are below you, while a cliff drops off, right at the edge.";
                    	if (player in vplat_seats) "Your feet dangle over the edge of a cliff the drops off a thousand feet.";
                    	if (player in cavern) <<Examine lc_space>>;
                    	"You see nothing unexpected in that direction.";
                    ],
                    before [;
                        Stand:
                            if (player in berth) return ExitBerth();
                    	...
                    	Touch:
                    		if (player in berth) "You need to get out of your berth to do that.";
                    		if (player in crew_quarters or main_junction or galley or radar_room or under_desk or bridge or armory or armory_backroom or main_airlock or cargo_hold or hidden_compartment or secondary_junction or engine_room) <<Touch ship_plating>>;
                    ];
...
[ LanguageLM n x1;
  Answer,Ask:
            "There is no reply.";
! Ask:      see Answer
  Attack:   "I've known strange people, but fighting ", (a) x1, "?";
  ...
  Fill:     "But there's nothing drinkable here.";
  FullScore: switch (n) {
  ...
  Miscellany: switch (n) {
        ...
        5:  print "^According to the Treaty of Amalasis (ratified after the third Vlaxion Conflict) you have the right to RESTART, RESTORE a saved game";
...
! Push: see Pull
  ...
  Rub:      "Lacking a multi-purpose scrub brush, you achieve nothing by this.";
  Save: switch (n) {
        1:  "Save failed.";
        2:  "Ok.";
    }
...
! Tie:  see JumpOver.
  Touch: switch (n) {
        1:  "Keep your hands to yourself!";
        2:  if (uniform in spacesuit or mal_spacesuit) return PCF();
